...
# 2. Add a Marker2D node named 'LootSpawnPoint' to your Player scene tree.
# Place it visually above the player's head in the 2D editor.
...
func spawn_loot_notification(item_data: ItemData):
	if loot_notifier_scene != null and loot_spawn_point != null:
		...
		# ADD THIS LINE: Tell all existing notifications to move up!
...
func _physics_process(delta):
	...
	# If no keys are pressed, stop instantly
	if direction == Vector2.ZERO:
		velocity = Vector2.ZERO
	else:
		# Smoothly transition current velocity to the target_velocity
...
func update_animation():
	...
	# If moving at base SPEED, scale is 1.0 (12 FPS). 
	# If sprinting faster, scale goes up (e.g., 1.66x faster).
...
func _unhandled_input(event):
	...
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
		...
		# Define the pool of possible items
